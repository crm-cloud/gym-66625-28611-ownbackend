...
import { useDashboardStats, useRevenueAnalytics, useMembershipAnalytics, useClassPopularity } from '@/hooks/useAnalytics';
import { useBranchContext } from '@/contexts/BranchContext';

export default function AnalyticsPage() {
  const { currentBranchId } = useBranchContext();
  
  const { data: stats } = useDashboardStats({ branchId: currentBranchId || undefined });
  const { data: revenueData = [] } = useRevenueAnalytics({ branchId: currentBranchId || undefined });
  const { data: membershipData = [] } = useMembershipAnalytics({ branchId: currentBranchId || undefined });
  const { data: classAttendanceData = [] } = useClassPopularity({ branchId: currentBranchId || undefined });
  return (
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold">{stats?.classAttendance || 0}%</p>
                <p className="text-sm text-green-600 flex items-center mt-1">
                  <TrendingUp className="w-4 h-4 mr-1" />
                  +2.1% from last month
                </p>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold">{stats?.memberRetention || 0}%</p>
                <p className="text-sm text-green-600 flex items-center mt-1">
                  <TrendingUp className="w-4 h-4 mr-1" />
                  +1.5% from last month
                </p>
